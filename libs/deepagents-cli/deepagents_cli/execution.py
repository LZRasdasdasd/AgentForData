...
def prompt_for_tool_approval(
    ...
    if platform.system() == "Windows":
        ...
        try:
            ...
            while True:
                if not first_render:
                    # Move cursor back to start of menu (up len(options) lines, then to start of line)
                    sys.stdout.write(f"\033[{len(options)}A\r")
                else:
                    # Only for first render, ensure we start at a new line
                    sys.stdout.write("\r")
                ...
                for i, option in enumerate(options):
                    # Clear current line completely before writing
                ...
                if msvcrt.kbhit():
                    ...
                    # Handle arrow keys (special keys start with \x00 or \xe0)
                    if char in {b'\x00', b'\xe0'}:
                        # Extended key code for arrow keys
                        extended_char = msvcrt.getch()
                        if extended_char == b'H':  # Up arrow
                            selected = (selected - 1) % len(options)
                        elif extended_char == b'P':  # Down arrow
                            selected = (selected + 1) % len(options)
                    elif char == b'\r':  # Enter
        ...
        try:
            ...
            # Fallback for systems where termios fails
