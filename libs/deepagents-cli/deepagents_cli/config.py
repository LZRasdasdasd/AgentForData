...
def create_model() -> BaseChatModel:
    ...
    Returns:
        ChatModel instance (DashScope, Anthropic, or Google)
    ...
    if settings.has_dashscope:
        try:
            from langchain_community.chat_models.dashscope import ChatDashScope # type: ignore
            model_name = os.environ.get("DASHSCOPE_MODEL", "qwen-turbo")
            base_url = os.environ.get("DASHSCOPE_BASE_URL")
            console.print(f"[dim]Using DashScope model: {model_name}[/dim]")
            kwargs = {"model": model_name, "temperature": 0.7}
            if base_url:
                kwargs["base_url"] = base_url
            return ChatDashScope(**kwargs)
        except ImportError:
            # Fallback to using dashscope directly if langchain_community doesn't have ChatDashScope
            from langchain_openai import ChatOpenAI
            
            model_name = os.environ.get("DASHSCOPE_MODEL", "qwen-max")
            base_url = os.environ.get("DASHSCOPE_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
            console.print(f"[dim]Using DashScope model via OpenAI compatibility: {model_name}[/dim]")
    ...
    console.print("  - DASHSCOPE_API_KEY     (for Qwen/千问 models)")
    console.print("  - ANTHROPIC_API_KEY     (for Claude models)")
    console.print("  - GOOGLE_API_KEY        (for Google Gemini models)")
    console.print("\nExample:")
    console.print("  export DASHSCOPE_API_KEY=your_api_key_here")
